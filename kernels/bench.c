...
int main(void) {
    ...
    #define ALIGN_UP(sz) (((size_t)(sz) + 63) & ~(size_t)63)
    block_q4_0 *x = aligned_alloc(64, ALIGN_UP((size_t)ROWS * nb * sizeof(block_q4_0)));
    block_q8_0 *y = aligned_alloc(64, ALIGN_UP((size_t)nb * sizeof(block_q8_0)));
    if (!x || !y) { printf("{\"ok\":false,\"error\":\"alloc failed\"}\n"); return 1; }
    ...
    printf("{\"ok\":%s,\"max_abs_err\":%.3e,\"max_rel_err\":%.3e,"
