...
| **8–20s** | Click **Run comparison**. Both panels start. Camera centered on the two live metric strips. Cerebras TTFT ~90ms, tok/s climbing into the hundreds; GPU still at its first tokens. | "Same question, same Gemma model, two engines. Cerebras streams the full analysis; the GPU is still on its first paragraph." |
| **20–35s** | Cerebras side completes the comparison. Pan down: the **bond knowledge graph** lights up — SpaceX '27 and '30 highlighted, similarity edges fanning out to comparable bonds. GPU side still streaming. | "Cerebras finishes a complete, graphed answer — placing both bonds against their peers — before the GPU finishes a sentence." |
